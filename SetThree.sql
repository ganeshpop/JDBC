...
INSERT INTO ratings(name, movie_id, rating)
VALUES ('James', 1, 8.4),
       ('James', 8, 8.6),
       ('Tim', 1, 9.0),
       ('James', 1, 7.6),
...
SELECT r.name AS user, r.rating
FROM ratings r,
     movies m
WHERE m.name = 'Spider-Man'
  AND r.movie_id = m.id
ORDER BY r.name;
SELECT r.name AS user, m.name AS movie, r.rating
FROM ratings r,
     movies m
WHERE r.rating > 5
  AND r.movie_id = m.id
...
SELECT r.name AS user, m.name AS movie, r.rating
FROM movies m
         JOIN rating.ratings r ON m.id = r.movie_id;
...
FROM movies m,
     ratings r
WHERE r.name = 'Zack'
  AND movie_id = (SELECT id FROM movies WHERE name = 'Spider-Man');

SELECT r.name as user, m.name as movie, r.rating
FROM ratings r,
     (SELECT id as mid, name FROM movies WHERE name = 'Spider-Man') m
WHERE r.name = 'Zack'
  AND r.movie_id = mid;

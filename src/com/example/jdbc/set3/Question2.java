...
public class Question2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            PreparedStatement insertStatement = connection.prepareStatement("INSERT INTO movies(name, release_year) VALUES (?, ?);");
            ...
            insertStatement = connection.prepareStatement("INSERT INTO ratings(name, movie_id, rating) VALUES ( ?, ?, ?);");
            for (int i = 0; i < numberOfItems; i++) {
                scanner.nextLine(); // For consuming newline character
                insertStatement.setString(1, scanner.nextLine());
                insertStatement.setInt(2, scanner.nextInt());
            ...
            PreparedStatement getCount = connection.prepareStatement("SELECT COUNT(*) AS COUNT FROM movies;");
            ResultSet resultSet = getCount.executeQuery();
            if (resultSet.next()) System.out.println("\n" + resultSet.getInt("COUNT"));
            getCount = connection.prepareStatement("SELECT COUNT(*) AS COUNT FROM ratings;");
